...
import numpy as np
import pandas as pd
from model import NN_autoenc
from tqdm import tqdm, trange
from test_autoenc import test
...
def train(name, filename, epochs, batch_size, learning_rate, device):
    # Create model folder
    model_path = './model/' + name + '_autoenc'
    ...
    model = NN_autoenc((3, 80, 128)).to(device)
    ...
    t2_length = float(len(train_2_dataset))
    ...
    for _ in range(5):
        ...
        steps_logger = trange(int(t1_length) // batch_size, desc='pre step')
        for t1_data in t1_generator:
            ...
            pred_img = model(im_train)
            loss = loss_function(pred_img, im_train)

            loss.backward()
            optimizer.step()

            steps_logger.set_postfix_str('Reconstruction: %.4f' % loss.item())
            steps_logger.update()
        steps_logger.close()
        print
    ...
    for epoch in epochs_logger:
        t1_generator = train_dataset.batches(batch_size, shuffle=True)
        t2_generator = train_2_dataset.batches(batch_size, shuffle=True)
        ...
        for t2_data in t2_generator:
            ...
            pred_img, pred = model(im_train, predict=True)
            loss = loss_function(pred_img, im_train)
            loss_pos = loss_function(pred, y_train)

            (0.0001 * loss + (t1_length / t2_length) * loss_pos).backward()
            optimizer.step()

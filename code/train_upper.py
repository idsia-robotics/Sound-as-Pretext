...
def train(name, filename, epochs, batch_size, learning_rate, device):
    ...
    model = NN(out_channels=3).to(device)
    ...
    training_steps = (len(train_dataset) + len(train_2_dataset)) // batch_size
    epochs_logger = trange(1, epochs + 1, desc='epoch')
    best_val_loss = 1e9

    for epoch in epochs_logger:
        generator = chain(train_dataset.batches(batch_size, shuffle=True),
                          train_2_dataset.batches(batch_size, shuffle=True))
        ...
        for step, data in enumerate(steps_logger, start=1):
            y_train = data['optitrack_tello_proj']
            im_train = data['rm_s1_camera_image_h264']
